...
//从就绪队列取一个任务
		...
		if(!clr_q) {
			conn=get_SC_connect(TCBno,poolno,0); //等待得到连接
			if(conn == (T_Connect *)-1) {
				clr_q=1; //清空队列
				conn=NULL;
		...
		if(ret<0) {
			if(conn) release_SC_connect(&conn,TCBno,poolno);
...
pthread_t *tp;
	...
	ShowLog(3,"%s:get pool[%d],tid:%lu,USEC=%llu",__FUNCTION__,poolno,pthread_self(),now_usec());
